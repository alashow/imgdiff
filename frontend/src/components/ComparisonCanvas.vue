<template>
  ...
  >
    <!-- Floating Controls -->
    <div
      class="absolute top-4 right-4 z-20 flex items-center gap-1.5 bg-slate-950/90 border border-slate-800 p-1.5 rounded-xl shadow-lg backdrop-blur"
    >
      <!-- Reset pan/zoom -->
      <button
        @click="resetStageView"
        class="p-1.5 hover:bg-slate-800 rounded-lg text-slate-400 hover:text-white transition-all"
      ...
      <button
        @click="toggleScaleMatch"
        :class="scaleMatchActive ? 'text-indigo-400 hover:text-indigo-300' : 'text-slate-400 hover:text-white'"
        class="p-1.5 hover:bg-slate-800 rounded-lg text-xs font-semibold px-2 flex items-center gap-1 transition-all"
        :title="scaleMatchActive ? 'Fit window enabled' : 'Original size mode enabled'"
      >
        <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <button
        ...
        :class="isDragLocked ? 'text-amber-400 hover:text-amber-300' : 'text-slate-400 hover:text-white'"
        class="p-1.5 hover:bg-slate-800 rounded-lg transition-all"
        :title="isDragLocked ? 'Drag locked — click to enable pan' : 'Drag enabled — click to lock'"
      >
        <!-- Locked icon -->
        <svg
          v-if="isDragLocked"
    ...
    >
        ...
        <div v-if="compareMode === 'slider'" class="absolute inset-0 flex items-center justify-center p-8">
          <div class="pan-container origin-center select-none" :style="{ transform: transformStyle }">
            <div ref="sliderWrapper" class="diff-wrapper shadow-2xl transition-all">
              <img
                v-if="currentPair.urlA"
                :src="currentPair.urlA"
                @load="onLoadA"
                class="diff-img rounded"
                :style="scaleMatchActive ? fitImageStyle : undefined"
              />
              <img
                v-else-if="currentPair.urlB"
                :src="currentPair.urlB"
                @load="onLoadB"
                class="diff-img rounded"
                :style="scaleMatchActive ? fitImageStyle : undefined"
              />
              <img
                v-if="hasBothCompareImages"
                :src="currentPair.urlB"
                @load="onLoadB"
                class="diff-img diff-img-b rounded"
                :style="[
              ...
              <div
                v-if="hasBothCompareImages"
                class="absolute top-0 bottom-0 w-0.5 bg-white cursor-ew-resize flex items-center justify-center pointer-events-auto z-10"
                :style="{ left: `${sliderPos}%`, transform: 'translateX(-50%)' }"
                @mousedown.stop="startSlideDrag"
                @touchstart.stop.prevent="startSlideDrag"
              >
          ...
          <div class="pan-container origin-center select-none" :style="{ transform: transformStyle }">
            <div class="diff-wrapper shadow-2xl">
              <img
                ...
                :src="currentPair.urlA"
                @load="onLoadA"
                class="diff-img rounded opacity-100 mix-blend-normal"
                :style="scaleMatchActive ? fitImageStyle : undefined"
              />
              <img
                v-else-if="currentPair.urlB"
            ...
            <div class="flex flex-col items-center gap-3">
              ...
              <div class="shadow-2xl flex items-center justify-center relative">
                <img
                  :src="currentPair.urlA"
                  @load="onLoadA"
                  class="diff-img rounded border border-slate-700/40"
                  :style="scaleMatchActive ? fitSideImageStyle : originalSideImageStyle"
                />
              </div>
            </div>
            <div class="flex flex-col items-center gap-3">
      ...
      <div v-else class="text-center flex flex-col items-center gap-2">
        <svg class="w-16 h-16 text-slate-700" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    >
      <svg class="w-16 h-16 text-indigo-400 animate-bounce" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
function handleTouchStart(e: TouchEvent) {
  ...
  if (e.touches.length !== 1 || isDragLocked.value) return;
  const touch = e.touches[0];
  isPanning.value = true;
  startMouseX.value = touch.clientX - panX.value;
  startMouseY.value = touch.clientY - panY.value;
}

function handleTouchMove(e: TouchEvent) {
  if (e.touches.length === 2 && pinchStartDistance.value > 0) {
    const distance = getTouchDistance(e.touches[0], e.touches[1]);
    const factor = distance / pinchStartDistance.value;
    zoom.value = Math.max(0.1, Math.min(pinchStartZoom.value * factor, 25));
    return;
  }

  if (!isPanning.value || e.touches.length !== 1) return;
  const touch = e.touches[0];
  const nx = touch.clientX - startMouseX.value;
  const ny = touch.clientY - startMouseY.value;
  if (rafPan) return;
  rafPan = requestAnimationFrame(() => {
    panX.value = nx;
    panY.value = ny;
...
function startSlideDrag(e: MouseEvent | TouchEvent) {
  e.preventDefault();
  ...
  const moveTouch = (event: TouchEvent) => {
    ...
    updateSliderFromClientX(event.touches[0].clientX);
  };
